...
img, threshold = adaptive_threshold('../output/image3_disconnected.png')
...
for i in range(0,len(y_set)):
        junctions.append( list() ) #different object reference each time

c=len(joints)-1
for i in range(len(y_set)):
    for j in range(len(x_set)):
        if((x_set[j],y_set[i])==(joints[c])):
            junctions[i].append(joints[c])
            c-=1
        else:
           junctions[i].append((-1,-1)) 
